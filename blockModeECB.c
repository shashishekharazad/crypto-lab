...
int main()
{
    ...
    len = strlen(message);
    unsigned char cipheredMessage[len];
    unsigned char decipheredMessage[len];
    ...
    for (i = 0; i < len / 16; i++)
    {
        ...
        strcat(cipheredMessage, ciphertext);
        // free(ciphertext);
        // free(plainText);
    }
    ...
    for (i = 0; i < len / 16; i++)
    {
        ...
        for (j = 0; j < 16; j++)
        {
            ciphertext[j] = cipheredMessage[s++];
        }
        aesDecryption(ciphertext, decryptedtext, key, size);
        puts(decryptedtext);
        strcat(decipheredMessage, decryptedtext);
        // free(ciphertext);
        // free(decryptedtext);
    }

    printf("\n\nEnciphered Text : ");
    puts(cipheredMessage);

    printf("\n\nDeciphered Text: ");
    puts(decipheredMessage);
