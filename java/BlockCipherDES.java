...
class BlockCipherDES {
    ...
    // Array to store the number of circular left shift that are to be done on each
    // round
    ...
    public String des(String input, String key, boolean choice) {
        int inputBits[] = new int[64];
        if (choice == false) {
            byte in_num[] = input.getBytes();
            String num_binary = "";
            for (int i = 0; i < in_num.length; i++) {
                String bin = Integer.toBinaryString(in_num[i]);
                num_binary = num_binary + "0".repeat(8 - bin.length()) + bin;
            }

            for (int i = 0; i < 64; i++) {
                inputBits[i] = num_binary.charAt(i) - '0';
            }
            for (int a : inputBits) {
            }
        } else {
            for (int i = 0; i < input.length(); i++) {
                inputBits[i] = input.charAt(i) - '0';
            }
        }
        int keyBits[] = new int[64];
        byte key_num[] = key.getBytes();
        String key_binary = "";
        for (int i = 0; i < key_num.length; i++) {
            String bin = Integer.toBinaryString(key_num[i]);
            key_binary = key_binary + "0".repeat(8 - bin.length()) + bin;
        }
        for (int i = 0; i < 64; i++) {
            keyBits[i] = key_binary.charAt(i) - '0';
        }
        for (int a : keyBits) {
        }

        return permute(inputBits, keyBits, choice);
    }
    ...
    private static String permute(int[] inputBits, int[] keyBits, boolean isDecrypt) {
        ...
        for (i = 0; i < 64; i++) {
            ...
            binary_str += finalOutput[i];
        }
        String finalOut_Str = "";
        for (int j = 0; j < 64; j++) {
            finalOut_Str += finalOutput[j];
        }
        String ans = "";
        for (int j = 0; j < 8; j++) {
            ans += (char) Integer.parseInt(binary_str.substring(0, 8), 2);
            binary_str = binary_str.substring(8);
        }
        if (isDecrypt) {
    ...
    public String xorStrings(String s1, String s2, boolean isBinary) {
